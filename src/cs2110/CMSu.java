...
/*
 * Assignment metadata
 * Name and NetID: Cam Brady cab495
...
public class CMSu {
    ...
    public boolean hasConflict(Student student) {
        assert checkCreditConsistency();

        for(int i = 0; i < nStudents; i++) {
            assert students[i].equals(student);
        }
    ...
    public StudentSet auditCredits(int creditLimit) {
        ...
        StudentSet over = new StudentSet();
    ...
    public boolean checkCreditConsistency() {
        ...
        int i = 0;
        while (match && i < nStudents) {
            int sum = 0;

            for(int j = 0; j < nCourses; j++) {
                if(courses[j].hasStudent(students[i])) {
                    sum += courses[j].credits();
                }
            }

            if(sum != students[i].credits()) {
                match = false;
            }
